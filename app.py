...
def display_option():
    ...
    if request.method == 'POST':
        ...
        if option == 'option2':
            ...
            return redirect(url_for('display_story'))
    return render_template('options.html', option_text = op['option_text'], option1 = op['opt_1'] , option2 = op['opt_2'] ,username = session['username'], score = score)
...
def leaderboard():
    ...
    return render_template("leaderboard.html", users = users_list, scores = scores_list, ranks = rank_list, truser = user, userrank = user_rank)
